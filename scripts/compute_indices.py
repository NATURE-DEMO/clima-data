...
def load_and_process_ensemble(
    ...
    if indicators:
        ...
        if any(idx in memory_intensive_indices for idx in current_indices):
            chunk_config["x"] = min(chunk_config["x"] // 2, 50)  # Even smaller spatial chunks
            chunk_config["y"] = min(chunk_config["y"] // 2, 50)
            chunk_config["realization"] = 1  # Process one realization at a time
            print(f"Using smaller chunks for memory-intensive indices: {chunk_config}")
